    ...
    <div className="flex-1 space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950/60 to-purple-950/40 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center space-x-3">
            <div className="p-3 bg-indigo-600/30 border border-indigo-500/40 rounded-2xl text-indigo-300 shadow-inner">
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                disabled={loading}
                className="w-full pl-12 pr-4 py-3.5 bg-slate-950/90 text-slate-100 placeholder-slate-500 border border-slate-800 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-inner"
              />
            </div>
            <button
              type="submit"
              disabled={loading || !topic.trim()}
              className="px-7 py-3.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:to-pink-500 text-white font-bold text-sm rounded-2xl shadow-lg shadow-indigo-600/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 shrink-0"
            >
              {loading ? (
                <>
          ...
          <div className="flex items-center flex-wrap gap-2 pt-2">
            ...
            {PRESET_TOPICS.map((preset, idx) => (
              <button
                ...
                disabled={loading}
                className="text-xs bg-slate-950/80 hover:bg-slate-800 text-slate-300 hover:text-indigo-300 border border-slate-800 hover:border-indigo-500/50 px-3 py-1 rounded-xl transition-all"
              >
                {preset}
              </button>
            ))}
      ...
      {studyData ? (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900/80 backdrop-blur-xl border border-slate-800 p-2 rounded-2xl shadow-lg">
            ...
            {/* Topic Badge */}
            <div className="hidden md:flex items-center space-x-2 bg-slate-950 px-3.5 py-1.5 rounded-xl border border-slate-800 text-xs text-indigo-300 font-medium">
              <span>Topic:</span>
              <strong className="text-white">{studyData.topic}</strong>
            </div>
          ...
          {activeTab === 'notes' && (
            <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-indigo-600/20"
                  >
              ...
              {/* Notes Body Content */}
              <div className="bg-slate-950/80 p-6 rounded-2xl border border-slate-800/80 font-sans shadow-inner">
                ...
                {filteredPyqs.length > 0 ? (
                  filteredPyqs.map((pyq, index) => {
                    ...
                    return (
                      <div
                        key={pyq.id || index}
                        className="bg-slate-950/90 border border-slate-800/80 hover:border-slate-700 rounded-2xl p-5 transition-all space-y-4"
                      >
                        {/* Question Metadata Row */}
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center space-x-2">
                            <span className="w-7 h-7 rounded-xl bg-indigo-600/30 border border-indigo-500/40 text-indigo-300 font-extrabold text-xs flex items-center justify-center">
                              Q{pyq.id || index + 1}
                            </span>
                            ...
                            {pyq.marks && (
                              <span className="text-[10px] bg-slate-900 text-slate-300 border border-slate-800 px-2 py-0.5 rounded-full font-mono">
                                {pyq.marks}
                              </span>
                            )}
                          </div>
                        ...
                        <div className="pt-2">
                          <button
                            ...
                            className="flex items-center space-x-2 text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors"
                          >
                        ...
                        {/* Solution Panel */}
                        {isExpanded && (
                          <div className="mt-3 pt-4 border-t border-slate-800 space-y-3 bg-slate-900/60 p-4 rounded-xl">
                            ...
                            {pyq.examTip && (
                              <div className="bg-amber-950/30 border border-amber-800/40 rounded-xl p-3 flex items-start space-x-2">
                                <Lightbulb className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                                <div className="text-[11px] text-amber-200">
                                  <strong className="font-bold text-amber-400">Examiner's Scoring Tip: </strong>
                                  {pyq.examTip}
                                </div>
                              </div>
                            )}
